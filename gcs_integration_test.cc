...
TEST(GcsIntegrationTest, LoadExtensionAndQueryGcs) {
  ...
  if (gcs_url_env == nullptr) {
    GTEST_SKIP() << "Skipping integration test because INTEGRATION_TEST_GCS_URL is not set.";
  }
  ...
  ASSERT_FALSE(ext_path.empty()) << "Could not find built extension shared library (libsqlite3_gcsvfs.so/dylib)";
  std::cout << "Loading extension from: " << ext_path << std::endl;

  // Load the extension
  char* err_msg = nullptr;
  rc = sqlite3_load_extension(db, ext_path.c_str(), "sqlite3_gcsvfs_init", &err_msg);
  ...
  ASSERT_EQ(rc, SQLITE_OK) << "Failed to open GCS database: " << (db ? sqlite3_errmsg(db) : "unknown error");
  ...
  std::cout << "Query completed successfully. Found " << row_count << " tables." << std::endl;
}
